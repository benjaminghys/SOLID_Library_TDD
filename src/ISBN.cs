...
public class ISBN
{
    ...
    ///     When the ISBN code is not 13 characters long or when it consists of non-numerical
    ///     characters.
    /// </exception>
    /// <remarks>For complexity’s sake ISBN are fixed to 13 numerical characters and no checksum will be performed.</remarks>
    public ISBN(string value)
    {
        if (value.Length != 13) throw new ArgumentException($"ISBN code is malformed. Expected 13, got {value.Length}");

        if (!IsDigitsOnly(value)) throw new ArgumentException($"ISBN consists of non numerical characters: {value}");
